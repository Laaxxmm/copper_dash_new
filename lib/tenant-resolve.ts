...
import { userById, clientById, type ControlUser } from './control-db';
import { runWithTenant, type TenantCtx } from './tenant';

export type Access = 'ok' | 'user-locked' | 'client-suspended';
export type SessionInfo = { userId: number; user: ControlUser; tenant?: TenantCtx; access: Access };
...
export const resolveSession = cache(async (): Promise<SessionInfo | null> => {
  try {
    ...
    if (user.client_id == null) { // global super-admin: no client → default DB
      return { userId: id, user, tenant: undefined, access: user.status === 'active' ? 'ok' : 'user-locked' };
    }
    ...
    return { userId: id, user, tenant, access };
  } catch {
    return null;
  }
});
